...
import static com.nmj.functions.PreferenceKeys.TRAKT_PASSWORD;
...
public class Trakt {
	...
	public static boolean performMovieCheckin(String tmdbId, Context c) {
		...
		try {
			...
			Request request = NMJLib.getTraktAuthenticationRequest("http://api.trakt.tv/movie/cancelcheckin/" + getApiKey(c), username, password);
		...
		try {
			...
			Request request = NMJLib.getJsonPostRequest("http://api.trakt.tv/movie/checkin/" + getApiKey(c), holder);
			Response response = NMJManagerApplication.getOkHttpClient().newCall(request).execute();
	...
	public static boolean performEpisodeCheckin(TvShowEpisode episode, Context c) {
		...
		try {
			// Cancel any current check-in
			Request request = NMJLib.getTraktAuthenticationRequest("http://api.trakt.tv/show/cancelcheckin/" + getApiKey(c), username, password);
			Response response = NMJManagerApplication.getOkHttpClient().newCall(request).execute();
			if (!response.isSuccessful())
				return false;
		} catch (Exception e) {
		...
		try {
			...
			Request request = NMJLib.getJsonPostRequest("http://api.trakt.tv/show/checkin/" + getApiKey(c), holder);
			Response response = NMJManagerApplication.getOkHttpClient().newCall(request).execute();
			return response.isSuccessful();
		} catch (Exception e) {
			return false;
	...
	public static boolean markMovieAsWatched(List<Movie> movies, Context c) {
		...
		try {
			...
			Request request = NMJLib.getJsonPostRequest((movies.get(0).hasWatched() ? "http://api.trakt.tv/movie/seen/" : "http://api.trakt.tv/movie/unseen/") + getApiKey(c), holder);
			Response response = NMJManagerApplication.getOkHttpClient().newCall(request).execute();
			return response.isSuccessful();
		} catch (Exception e) {
			return false;
	...
	public static boolean changeSeasonWatchedStatus(String showId, int season, Context c, boolean watched) {
		...
		try {
			...
			Request request = NMJLib.getJsonPostRequest("http://api.trakt.tv/show/season/" + (!watched ? "un" : "") + "seen/" + getApiKey(c), holder);
			Response response = NMJManagerApplication.getOkHttpClient().newCall(request).execute();
			return response.isSuccessful();
		} catch (Exception e) {
			return false;
	...
	public static boolean markEpisodeAsWatched(String showId, List<com.nmj.functions.TvShowEpisode> episodes, Context c, boolean watched) {
		SharedPreferences settings = PreferenceManager.getDefaultSharedPreferences(c);
		String username = settings.getString(TRAKT_USERNAME, "").trim();
		String password = settings.getString(TRAKT_PASSWORD, "");
		...
		try {
			...
			Request request = NMJLib.getJsonPostRequest("http://api.trakt.tv/show/episode/" + (!watched ? "un" : "") + "seen/" + getApiKey(c), holder);
			Response response = NMJManagerApplication.getOkHttpClient().newCall(request).execute();
	...
	public static boolean markTvShowAsWatched(TraktTvShow show, Context c) {
		...
		try {
			...
			Request request = NMJLib.getJsonPostRequest("http://api.trakt.tv/show/episode/seen/" + getApiKey(c), json);
			Response response = NMJManagerApplication.getOkHttpClient().newCall(request).execute();
			return response.isSuccessful();
		} catch (Exception e) {
			return false;
	...
	public static boolean addMoviesToLibrary(List<Movie> movies, Context c) {
		...
		try {
			...
			Request request = NMJLib.getJsonPostRequest("http://api.trakt.tv/movie/library/" + getApiKey(c), json);
			Response response = NMJManagerApplication.getOkHttpClient().newCall(request).execute();
			return response.isSuccessful();
		} catch (Exception e) {
			return false;
	...
	public static boolean movieWatchlist(List<Movie> movies, Context c) {
		...
		try {
			...
			Request request = NMJLib.getJsonPostRequest((movies.get(0).toWatch() ? "http://api.trakt.tv/movie/watchlist/" : "http://api.trakt.tv/movie/unwatchlist/") + getApiKey(c), json);
			Response response = NMJManagerApplication.getOkHttpClient().newCall(request).execute();
			return response.isSuccessful();
		} catch (Exception e) {
			return false;
	...
	public static boolean addTvShowToLibrary(TraktTvShow show, Context c) {
		...
		try {
			...
			Request request = NMJLib.getJsonPostRequest("http://api.trakt.tv/show/episode/library/" + getApiKey(c), json);
			Response response = NMJManagerApplication.getOkHttpClient().newCall(request).execute();
			return response.isSuccessful();
		} catch (Exception e) {
			return false;
	...
	public static boolean tvShowFavorite(List<TvShow> shows, Context c) {
		...
		try {
			...
			Request request = NMJLib.getJsonPostRequest("http://api.trakt.tv/rate/shows/" + getApiKey(c), json);
			Response response = NMJManagerApplication.getOkHttpClient().newCall(request).execute();
			return response.isSuccessful();
		} catch (Exception e) {
			return false;
	...
	public static JSONArray getMovieLibrary(Context c, int type) {
		...
		try {
			...
			Request request = NMJLib.getTraktAuthenticationRequest(url, username, password);
			Response response = NMJManagerApplication.getOkHttpClient().newCall(request).execute();
	...
	public static Show getShowSummary(Context context, String showId) {
		return new Show(NMJLib.getJSONObject(context, "http://api.trakt.tv/show/summary.json/" + getApiKey(context) + "/" + showId));
	}
	
	public static com.nmj.apis.trakt.Movie getMovieSummary(Context context, String movieId) {
		return new com.nmj.apis.trakt.Movie(NMJLib.getJSONObject(context, "http://api.trakt.tv/movie/summary.json/" + getApiKey(context) + "/" + movieId));
	}
}