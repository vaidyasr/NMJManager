...
public class FileMovie extends MovieFileSource<File> {
	...
	public void addToResults(File file, TreeSet<String> results) {
		if (supportsNfo() && NMJLib.isNfoFile(file.getAbsolutePath())) {
			try {
				addNfoFile(NMJLib.removeExtension(file.getAbsolutePath()), new FileInputStream(file));
			} catch (FileNotFoundException ignored) {}
		} else if (NMJLib.checkFileTypes(file.getAbsolutePath())) {
