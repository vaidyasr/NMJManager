...
public class UpnpMovie extends MovieFileSource<String> {
	...
	public void removeUnidentifiedFiles() {
		...
		for (int i = 0; i < count; i++) {
			if (dbMovies.get(i).isUpnpFile() && dbMovies.get(i).isUnidentified() && NMJLib.exists(dbMovies.get(i).getFilepath()))
	...
	public void removeUnavailableFiles() {
		...
		int count = dbMovies.size();
		for (int i = 0; i < count; i++) {
			if (dbMovies.get(i).isUpnpFile() && !NMJLib.exists(dbMovies.get(i).getFilepath())) {
                MovieDatabaseUtils.deleteMovie(mContext, dbMovies.get(i).getTmdbId());
			}
		}
	}
	...
	public List<String> searchFolder() {
		DbAdapterMovieMappings dbHelper = NMJManagerApplication.getMovieMappingAdapter();
