...
import com.nmj.smbstreamer.Streamer;
...
import static com.nmj.functions.PreferenceKeys.IGNORE_VIDEO_FILE_TYPE;

public class VideoUtils {
	
	// TODO Move all related methods from NMJLib, etc. into this class
	...
	private static boolean playNetworkFile(final Activity activity, final String filepath, final Object videoObject) {
		...
		if (!NMJLib.isWifiConnected(activity)) {
		...
		int contentType = (videoObject instanceof Movie) ? NMJLib.TYPE_MOVIE : NMJLib.TYPE_SHOWS;
		final SmbLogin auth = NMJLib.getLoginFromFilepath(contentType, filepath);
		...
		new Thread(){
			public void run(){
				try{
					...
					s.setStreamSrc(file, NMJLib.getSubtitleFiles(filepath, auth)); //the second argument can be a list of subtitle files
