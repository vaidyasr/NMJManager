...
public class MovieStructure {
	...
	public void checkImdbId() {
		...
		temp = NMJLib.decryptImdbId(getParentFolderName());
		if (null != temp) {
			mImdbId = temp;
		}
	}
	...
	public String getDecryptedFilename() {
		return NMJLib.decryptName(NMJLib.getFilenameWithoutExtension(getFilename()), mCustomTags);
	}
	
	public String getDecryptedParentFolderName() {
		return NMJLib.decryptName(getParentFolderName(), mCustomTags);
	}
	...
	public int getReleaseYear() {
		...
		while (matcher.find())
			result = NMJLib.getInteger(matcher.group(1));
