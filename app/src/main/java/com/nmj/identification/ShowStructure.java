...
public class ShowStructure {
	...
	public void checkImdbId() {
		...
		if (hasShowFolder()) {
			String temp = NMJLib.decryptImdbId(getShowFolderName());
			if (null != temp) {
				mImdbId = temp;
				return;
			}
		...
		String temp = NMJLib.decryptImdbId(getFilename());
		if (null != temp)
			mImdbId = temp;
	}
	...
	public String getDecryptedFilename() {
		if (mEpisodes.size() > 0)
			return NMJLib.decryptName(mEpisodes.get(0).getBefore(), mCustomTags);
		return NMJLib.decryptName(getFilename(), mCustomTags);
	}
	...
	public String getDecryptedShowFolderName() {
		return NMJLib.decryptName(getShowFolderName(), mCustomTags);
	}
	...
	public int getSeasonFolderNumber(String folderName) {
		...
		if (matcher.find())
			return NMJLib.getInteger(matcher.group(1));
	...
	public ArrayList<Episode> decryptEpisodes(String filename) {
		...
		if (matcher.find()) {			
			...
			if (count > 1) {
				...
				while (matcher.find()) {
					...
					int season = NMJLib.getInteger(matcher.group(2));
					int episode = NMJLib.getInteger(matcher.group(3));
				...
				while (matcher.find()) {
					...
					for (String episode : splitPattern.split(matcher.group(3)))
						episodes.add(new Episode(season, NMJLib.getInteger(episode), before, after));
				}
			}
		...
		if (matcher.find()) {
			int season = NMJLib.getInteger(matcher.group(1));
			int episode = NMJLib.getInteger(matcher.group(2));
			String after = filename.substring(matcher.end(), filename.length());
		...
		if (matcher.find()) { // We only want a single set of results here	
			...
			case 3: // ### (season [1], episode [2])
				season = NMJLib.getInteger(group.substring(0, 1));
				episode = NMJLib.getInteger(group.substring(1, 3));
				break;
			case 4: // #### (season [2], episode [2])
				season = NMJLib.getInteger(group.substring(0, 2));
				episode = NMJLib.getInteger(group.substring(2, 4));
				break;
			case 5: // ##### (season [2], episode [3])
				season = NMJLib.getInteger(group.substring(0, 2));
				episode = NMJLib.getInteger(group.substring(2, 5));
				break;
			case 6: // ###### (season [3], episode [3])
				season = NMJLib.getInteger(group.substring(0, 3));
				episode = NMJLib.getInteger(group.substring(3, 6));
				break;
			case 7: // ####### (season [4], episode [3])
				season = NMJLib.getInteger(group.substring(0, 4));
				episode = NMJLib.getInteger(group.substring(4, 7));
				break;
			}
		...
		if (matcher.find()) { // We only want a single set of results here	
			...
			int episode = NMJLib.getInteger(matcher.group(1));
