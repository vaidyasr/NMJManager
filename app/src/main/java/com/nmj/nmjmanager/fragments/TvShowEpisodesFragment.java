...
import com.nmj.loader.TvShowEpisodeLoader;
import com.nmj.nmjmanager.IdentifyTvShowEpisode;
import com.nmj.nmjmanager.NMJManagerApplication;
import com.nmj.nmjmanager.R;
import com.nmj.nmjmanager.TvShowEpisodeDetails;
...
import static com.nmj.functions.PreferenceKeys.TVSHOWS_COLLECTION_LAYOUT;
...
public class TvShowEpisodesFragment extends Fragment {
	...
	@Subscribe
	public void refreshData(com.nmj.nmjmanager.TvShowEpisode episode) {
		loadEpisodes();
	}
	...
	public void onViewCreated(View v, Bundle savedInstanceState) {
		...
		if (NMJLib.isTablet(mContext) && !NMJLib.isPortrait(mContext)) {
		...
		mGridView.setMultiChoiceModeListener(new MultiChoiceModeListener() {
			...
			public boolean onActionItemClicked(ActionMode mode, MenuItem item) {
				...
				mBus.post(new com.nmj.nmjmanager.TvShowEpisode());
	...
	private void changeWatchedStatus(boolean watched, final Set<Integer> checkedEpisodes) {
		...
		for (int episode : checkedEpisodes) {
			db.setEpisodeWatchStatus(mShowId, NMJLib.addIndexZero(mSeason), NMJLib.addIndexZero(episode), watched);
		}

		if (NMJLib.isOnline(mContext) && Trakt.hasTraktAccount(mContext))
	...
	private void identify(final Set<Integer> checkedEpisodes) {
		...
		for (int episode : checkedEpisodes) {
			ArrayList<String> paths = NMJManagerApplication.getTvShowEpisodeMappingsDbAdapter().getFilepathsForEpisode(mShowId, NMJLib.addIndexZero(mSeason), NMJLib.addIndexZero(episode));
		...
		i.putExtra("showTitle", NMJManagerApplication.getTvDbAdapter().getShowTitle(mShowId));
		
		getActivity().startActivityForResult(i, 0);
	}

	private void syncWatchedStatusWithTrakt(final Set<Integer> checkedEpisodes, final boolean watched) {
		new com.nmj.functions.AsyncTask<Void, Boolean, Boolean>() {
	...
	private void removeSelectedEpisodes(final Set<Integer> selectedEpisodes) {
		...
		builder.setPositiveButton(android.R.string.yes, new DialogInterface.OnClickListener() {
			...
			public void onClick(DialogInterface dialog, int which) {
				...
				if (NMJManagerApplication.getTvEpisodeDbAdapter().getEpisodeCountForSeason(mShowId, NMJLib.addIndexZero(mSeason)) == 0) {
	...
	private class ImageAdapter extends BaseAdapter {
		...
		public View getView(final int position, View convertView, ViewGroup container) {
			...
			if (mUseGridView)
				...
				holder.subtext.setText(episode.getSubtitleText() + "\n" + NMJLib.getPrettyDatePrecise(mContext, episode.getAirDate()));
