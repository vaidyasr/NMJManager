...
public class WebVideoFragment extends Fragment {
		...
		mGridView.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> arg0, View arg1, int arg2, long arg3) {
				if (YouTubeApiServiceUtil.isYouTubeApiServiceAvailable(getActivity()).equals(YouTubeInitializationResult.SUCCESS)) {
					Intent intent = YouTubeStandalonePlayer.createVideoIntent(getActivity(), NMJLib.getYouTubeApiKey(getActivity()), mVideos.get(arg2).getId(), 0, false, true);
	...
	protected class GetYouTubeVideos extends AsyncTask<String, String, String> {
		...
		protected String doInBackground(String... params) {
			...
			try {
				JSONObject jObject = NMJLib.getJSONObject(mContext, "http://gdata.youtube.com/feeds/api/standardfeeds/most_popular?time=today&alt=json&start-index=1&max-results=50");
	...
	protected class RedditSearch extends AsyncTask<String, String, String> {
		...
		protected String doInBackground(String... params) {
			...
			try {
				JSONObject jsonObject = NMJLib.getJSONObject(mContext, "http://www.reddit.com/r/videos/hot.json?sort=hot&limit=100");
				...
				for (int i = 0; i < jsonArray.length(); i++) {
					if (jsonArray.getJSONObject(i).getJSONObject("data").getString("domain").equals("youtube.com") || jsonArray.getJSONObject(i).getJSONObject("data").getString("domain").equals("youtu.be")) {
						String youtubeId = NMJLib.getYouTubeId(jsonArray.getJSONObject(i).getJSONObject("data").getString("url"));
	...
	protected class TEDSearch extends AsyncTask<String, String, String> {
		...
		protected String doInBackground(String... params) {
			...
			try {
				JSONObject jObject = NMJLib.getJSONObject(mContext, "http://gdata.youtube.com/feeds/api/users/TEDtalksDirector/uploads?alt=json&start-index=1&max-results=50");
				JSONObject jdata = jObject.getJSONObject("feed");
				JSONArray aitems = jdata.getJSONArray("entry");
