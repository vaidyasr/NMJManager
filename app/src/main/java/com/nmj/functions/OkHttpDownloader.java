		...
		if (!ignoreNfo) {
			...
			if (imageUri.contains("movie")) {
				...
				if (!TextUtils.isEmpty(id)) {
					ArrayList<String> strings = NMJManagerApplication.getMovieMappingAdapter().getMovieFilepaths(id);
			...
			for (Filepath path : filepaths) {	
				...
				if (path.getType() == FileSource.SMB) {
                    if (NMJLib.isOnline(mContext)) {
                        if (!TextUtils.isEmpty(filename)) {
                            SmbFile s;

                            // Determine if this is a movie path or TV show path
                            if (imageUri.contains("movie")) {
                                s = NMJLib.getCustomCoverArt(filename, NMJLib.getLoginFromFilepath(NMJLib.TYPE_MOVIE, filename),
                                        imageUri.contains("movie-thumbs") ? NMJLib.COVER : NMJLib.BACKDROP);
