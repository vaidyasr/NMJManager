...
public abstract class MediumBaseMovie extends BaseMovie {
	...
	public MediumBaseMovie(Context context, String title, String tmdbId, String rating, String releasedate,
		...
		// Date added
		mDateAdded = NMJLib.getPrettyDate(mContext, Long.valueOf(getDateAdded()));
		
		// Runtime
		mRuntime = NMJLib.getPrettyTime(mContext, Integer.parseInt(getRuntime()));
		
		// Release date
		mReleaseDate = NMJLib.getPrettyDate(mContext, getReleasedate());
	}
	...
	public void setReleaseDate(int year, int month, int day) {
		RELEASEDATE = year + "-" + NMJLib.addIndexZero(month) + "-" + NMJLib.addIndexZero(day);
	}
	...
	public String getDateAdded() {
		if (TextUtils.isEmpty(DATE_ADDED) || !NMJLib.isNumber(DATE_ADDED))
			return "0";
		return DATE_ADDED;
	}
