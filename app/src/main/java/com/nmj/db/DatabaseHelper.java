...
public class DatabaseHelper extends SQLiteOpenHelper {
	...
	protected static final String TAG = "NMJManager";

	public static final String DATABASE_NAME = "NMJManager_data";
	...
	public void onUpgrade(SQLiteDatabase database, int oldVersion, int newVersion) {
		Log.d("NMJManager", "Upgrading database...");
		...
		if (oldVersion == 4) { // Upgrade to the new database structure
			...
			// Move TV show database and TV show episodes database to NMJManager_data database
			moveTvShowDb(NMJManagerApplication.getContext(), database);

			// Move file sources database to NMJManager_data database
			moveFileSourcesDb(NMJManagerApplication.getContext(), database);
		} else {
	...
	private void moveTvShowDb(Context context, SQLiteDatabase database) {
		File dbFile = context.getDatabasePath("NMJManager_tv_show_data");
		File dbFile2 = context.getDatabasePath("NMJManager_tv_show_episode_data");
	...
	private void moveFileSourcesDb(Context context, SQLiteDatabase database) {
		File dbFile = context.getDatabasePath("NMJManager_sources");
		if (dbFile.exists()) {
			// The old database file exists! Let's create
			// a table for it in the new combined database
			database.execSQL(DATABASE_CREATE_FILESOURCES);
