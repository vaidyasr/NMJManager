...
public class DownloadImageService extends IntentService {
	...
	protected void onHandleIntent(Intent intent) {
		...
		boolean isOnline = NMJLib.isOnline(this);
		final int stringId = isOnline ? R.string.addingCover : R.string.noInternet;
		
		new Handler(Looper.getMainLooper()).post(new Runnable() {            
			@Override
			public void run() {
				Toast.makeText(NMJManagerApplication.getContext(), stringId, Toast.LENGTH_LONG).show();
			}
		});
	...
	private void setup() {
		...
		case IMAGE_TYPE_TVSHOW_BACKDROP:
			...
			if (mContentId.startsWith("tmdb_")) {
				...
				mImageUrl = mImageUrl.replace(NMJLib.getBackdropThumbUrlSize(this), NMJLib.getBackdropUrlSize(this));
			} else {
	...
	private void download() {
		...
		NMJLib.downloadFile(mImageUrl, mDownloadPath);
	}
	...
	private void resize() {
		...
		NMJLib.resizeBitmapFileToCoverSize(this, mDownloadPath);
	}
