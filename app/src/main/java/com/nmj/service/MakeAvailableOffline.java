...
import com.nmj.nmjmanager.CancelOfflineDownload;
...
import static com.nmj.functions.PreferenceKeys.BUFFER_SIZE;
import static com.nmj.functions.PreferenceKeys.IGNORE_FILESIZE_CHECK;
...
public class MakeAvailableOffline extends IntentService {
	...
	protected void onHandleIntent(Intent intent) {
		...
		LocalBroadcastManager.getInstance(this).registerReceiver(mMessageReceiver, new IntentFilter("NMJManager-stop-offline-download"));
		...
		try {
			int size = NMJLib.getThumbnailNotificationSize(mContext);
			mBuilder.setLargeIcon(NMJManagerApplication.getPicasso(mContext).load(intent.getExtras().getString("thumb")).resize(size, (int) (size * 1.5)).get());
		} catch (IOException ignored) {}
		try {
			int width = NMJLib.getLargeNotificationWidth(mContext);
			int height = (int) (width / 1.78);
			mBuilder.setStyle(
					new NotificationCompat.BigPictureStyle()
					.setSummaryText(message)
					.bigPicture(NMJManagerApplication.getPicasso(getApplicationContext()).load(intent.getExtras().getString("backdrop")).resize(width, height).get())
					);
		} catch (IOException e) {}
	...
	private boolean checkIfNetworkFileExists() {
		if (mFileUrl.startsWith("http")) {
			return NMJLib.exists(mFileUrl);
		} else {
			mAuth = NMJLib.getLoginFromFilepath(mType, mFileUrl);
			try {
				mSmb = new SmbFile(
	...
	private boolean checkFilesize() {
		try {			
			long freeMemory = (long) (NMJLib.getFreeMemory() * 0.975);
			if (mFileUrl.startsWith("http"))
				mSize = NMJLib.getFileSize(new URL(mFileUrl));
			else
				mSize = mSmb.length();
