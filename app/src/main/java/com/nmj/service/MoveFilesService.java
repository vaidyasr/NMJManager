...
import com.nmj.nmjmanager.SplashScreen;
...
public class MoveFilesService extends IntentService {
	...
	private void movieBackdrop(File backdrop) {
		try {
			FileUtils.copyFile(backdrop, new File(NMJManagerApplication.getMovieBackdropFolder(this), backdrop.getName()));
		} catch (IOException e) {}
	}

	private void tvShowBackdrop(File backdrop) {
		try {
			FileUtils.copyFile(backdrop, new File(NMJManagerApplication.getTvShowBackdropFolder(this), backdrop.getName()));
		} catch (IOException e) {}
	}

	private void episodePhoto(File episode) {
		try {
			FileUtils.copyFile(episode, new File(NMJManagerApplication.getTvShowEpisodeFolder(this), episode.getName()));
		} catch (IOException e) {}
	}

	private void movieThumb(File thumb) {
		try {
			FileUtils.copyFile(thumb, new File(NMJManagerApplication.getMovieThumbFolder(this), thumb.getName()));
		} catch (IOException e) {}
	}

	private void tvShowThumb(File thumb) {
		try {
			FileUtils.copyFile(thumb, new File(NMJManagerApplication.getTvShowThumbFolder(this), thumb.getName()));
		} catch (IOException e) {}
	}
}