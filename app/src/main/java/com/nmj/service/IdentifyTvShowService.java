...
public class IdentifyTvShowService extends IntentService implements TvShowLibraryUpdateCallback {
	...
	private void setupList() {
		Cursor cursor = NMJManagerApplication.getTvShowEpisodeMappingsDbAdapter().getAllFilepaths(mOldShowId);
	...
	private void removeOldDatabaseEntries() {
		...
		boolean result = NMJManagerApplication.getTvShowEpisodeMappingsDbAdapter().deleteAllFilepaths(mOldShowId);
		if (result) {
			...
			File[] seasonPhotos = NMJManagerApplication.getTvShowSeasonFolder(this).listFiles();
			...
			File[] episodePhotos = NMJManagerApplication.getTvShowEpisodeFolder(this).listFiles();
